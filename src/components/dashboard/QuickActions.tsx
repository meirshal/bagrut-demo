...
import { ChallengeCategory, ExcellenceTier } from '@/types';
...
const RISK_LEVEL_CONFIG: {
  ...
  { level: 2, label: 'רמה 2 - סיכון', color: '#d97706', bgColor: '#fef3c7' },
  { level: 3, label: 'רמה 3 - סיכון גבוה', color: '#dc2626', bgColor: '#fee2e2' },
];

const EXCELLENCE_CONFIG: {
  tier: string;
  label: string;
  color: string;
  bgColor: string;
...
export function QuickActions() {
  const data = useMemo(() => {
    ...
    const excelDist = EXCELLENCE_CONFIG.map((ec) => {
      const count = allStudents.filter((s) => s.excellenceTier === ec.tier).length;
      return { ...ec, count, pct: ((count / allStudents.length) * 100).toFixed(1) };
    });
    const maxExcelCount = Math.max(...excelDist.map((e) => e.count));

    return { challengeStudents, classMap, riskDist, maxRiskCount, excelDist, maxExcelCount };
  }, []);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card className="rounded-xl shadow-md">
        <CardHeader className="pb-2">
          <CardTitle className="text-xs font-bold text-slate-700 flex items-center gap-2">
            <span className="inline-block w-1.5 h-5 bg-indigo-400 rounded-full" />
            התפלגות הצטיינות
          </CardTitle>
        </CardHeader>
        <CardContent className="p-3 pt-0 space-y-2.5">
          {data.excelDist.map((ed) => (
            <div key={ed.tier} className="space-y-1">
              <div className="flex items-center justify-between text-[10px]">
                <span className="font-medium text-slate-600">{ed.label}</span>
                <span className="font-bold text-slate-800 tabular-nums">
                  {ed.count} ({ed.pct}%)
                </span>
              </div>
              <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className="h-full rounded-full transition-all duration-500"
                  style={{
                    width: `${(ed.count / data.maxExcelCount) * 100}%`,
